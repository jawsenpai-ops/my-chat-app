...
import { Alert, FlatList, Image, Modal, Share, StyleSheet, Text, TextInput, TouchableOpacity, View } from "react-native";
  ...
  const renderPost = ({ item }: { item: Post }) => {
    ...
    return <View style={styles.postCard}>
      <View style={styles.postHeader}>
        <View style={styles.authorWrap}><View><Image source={{ uri: item.author.avatar }} style={styles.avatar} />{online && <View style={styles.onlineDot} />}</View><View><Text style={styles.authorName}>{item.author.name}</Text><Text style={styles.meta}>{online ? "Online" : "Offline"} · {new Date(item.createdAt).toLocaleString()}</Text></View></View>
